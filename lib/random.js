...
let tanks = ["twin", "sniper", "single", "gunner", "tri", "booster","destroyer", "pounder", "hexa", "octo", "rifle", "director", "overlord"];

  
  
  let smashers = ['basic', "twin", "sniper"];
let boosters = ['basic', "twin", "sniper"]
    switch(tank){
        case "norm":
            return tanks[Math.floor(Math.random() * tanks.length)]
            case "smash":
                return tanks[Math.floor(Math.random() * smashers.length)]
                case "boost":
                    return tanks[Math.floor(Math.random() * boosters.length)]
    }
}
